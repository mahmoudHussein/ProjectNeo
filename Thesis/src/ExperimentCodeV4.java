...
//				System.out.println(systemsSt);
	...
	public static ArrayList<String> GetStartEvents (String[][] ObjectInfo){
		ArrayList<String> startEvents = new ArrayList<String>();				
		for (int i = 0 ; i < ObjectInfo.length ;i++  ){
			if(ObjectInfo[i][1] != null && ObjectInfo[i][1].contains("START_EVENT")){
				startEvents.add(ObjectInfo[i][0]);
				ObjectInfo[i][4] = "1";
			}
			
		}
		System.out.println("num of start events = " + startEvents.size());
		return startEvents;
	}
	
	public static void ProcessNumbering(int position,String ID, String[][] ObjectInfo){
		for(int i =0; i < ObjectInfo.length;i++){
			if(ObjectInfo[i][0] != null && ObjectInfo[i][0].equals(ID)){
				ObjectInfo[i][4] = position +"";
			}
		}
	}
	
	public static ArrayList<String> SingleSequance(String[][] ObjectInfo, String[][] connectionsArray, String CurrentObjId, ArrayList<String> sequance, int elementCounter ){
		
		String currentObject = CurrentObjId;
		for(int k = 0; k < connectionsArray.length; k++){
			if(connectionsArray[k][0] != null && connectionsArray[k][0].equals(currentObject)){
				if(connectionsArray[k][2].equals("CT_ACTIV_1") || connectionsArray[k][2].equals("CT_IS_PREDEC_OF_1") ||
				connectionsArray[k][2].equals("CT_LEADS_TO_1")|| connectionsArray[k][2].equals("CT_CRT_1")){
					currentObject = connectionsArray[k][3];
					sequance.add(currentObject);
					elementCounter++;
					ProcessNumbering(elementCounter,currentObject,ObjectInfo);
					SingleSequance(ObjectInfo, connectionsArray, currentObject, sequance, elementCounter);
					break;
			}
...
//		System.out.println("Conv  "+ tempSequance);
	...
	public static String[][] MultiSequance(String[][] ObjectInfo, String[][] connectionsArray){
		ArrayList<String> startEvents = GetStartEvents(ObjectInfo);
		ArrayList<String> tempSequance = new ArrayList<String>();
		int elementCounter= 1;
		String[][] Sequances = new String [50][50];
		String currentStartEventID;
		String currentObject;
		for (int i = 0 ; i < startEvents.size(); i++){
			currentStartEventID = startEvents.get(i);
			tempSequance.add(currentStartEventID);
			tempSequance =  SingleSequance(ObjectInfo, connectionsArray, currentStartEventID, tempSequance, elementCounter);
			System.out.println("Conv  "+ tempSequance);
			Sequances= ArrayListConvereter(Sequances,tempSequance, i);
			tempSequance.clear();
			elementCounter = 1;
		}
		//printBoard(Sequances);
		return Sequances;
	}
	
	public static int NumberOfRuleConnections(String ID, String[][] connectionsArray){
		int numberOfConnections = 0;
		for(int i =0; i < connectionsArray.length; i++){
			if(connectionsArray[i][0] != null && connectionsArray[i][0].equals(ID) && connectionsArray[i][4] != null){
				numberOfConnections++;
			}
		}
	...
	public static int RecordsCounter(String[][] multiSeqs){
		int counter =0;
		for(int i=0; i < multiSeqs.length; i++){
			if(multiSeqs[i][0] != null){
				counter++;
			}
		}
		return counter;
	}
	public static String[][] ShiftRows(String[][] multiSeqs, int toBeShifted, int shiftStartingPoint){
		int recordsCounter = RecordsCounter(multiSeqs);
		if(toBeShifted >= 2){
		for(int i=recordsCounter-1; i > shiftStartingPoint; i--){
			for(int j=0; j< multiSeqs[i].length; j++){
				multiSeqs[i+(toBeShifted-1)][j] = multiSeqs[i][j];
				multiSeqs[i][j] = null;
			}
		}
		}
		...
		public static ArrayList<String> GetPreElementsforProcess(String[][] multiSeqs, int row, int BeforeColumnNum){
			ArrayList<String> preElements = new ArrayList<String>();
			for(int j = 0; j < BeforeColumnNum; j++){
				preElements.add(multiSeqs[row][j]);
			}
			return preElements;
		}
		
	public static String[][] ProcessSequance(String[][] multiSeqs, String[][]ModelInfo, String[][]objInfo, String[][] connectionsArray){
		int subprocessIndex;
		int RuleprocessIndexJ;
		String modelID = "";
		ArrayList<String> modelElements = new ArrayList<String>();
		ArrayList<String> subprocessElements = new ArrayList<String>();
		ArrayList<String> postSubprocessElements = new ArrayList<String>();
		ArrayList<String> preElements = new ArrayList<String>();
		ArrayList<String> processSeq = new ArrayList<String>();
		printBoard(multiSeqs);
		System.out.println("original board");
		for(int i = 0; i < multiSeqs.length; i++){
			for(int j = 0; j < multiSeqs[i].length; j++){
				if(multiSeqs[i][j] != null){
//					System.out.println("original board");
				if(isRuleIndex(multiSeqs[i][j], objInfo)){
					int NumberOfRuleConnections = NumberOfRuleConnections(multiSeqs[i][j], connectionsArray);
					System.out.println("number of rule connections is : " + NumberOfRuleConnections);
					multiSeqs= ShiftRows(multiSeqs, NumberOfRuleConnections, i);
					RuleprocessIndexJ = j;
					preElements = GetPreElementsforProcess(multiSeqs, i, RuleprocessIndexJ);
//					printArrayList(preElements);
					for(int k = 1; k <= NumberOfRuleConnections; k++){
						for(int l = i+ k; l < NumberOfRuleConnections ;l++){
//							System.out.println(" l is equal" + l);
							for(int t = 0; t < preElements.size(); t++ ){
//								System.out.println(" t is equal" + t);
								multiSeqs[l][t] = preElements.get(t);
							}
						}
					}
				}
				if(isSubprocessIndex(multiSeqs[i][j], objInfo)){
					System.out.println("subprocess found");
					modelID = GetModelID(multiSeqs[i][j], objInfo);
//					System.out.println("model ID: " + modelID);
					modelElements = getModelElements(modelID, ModelInfo);
					printArrayList(modelElements);
					subprocessElements = getSubprocessElements(modelElements, multiSeqs);
					subprocessIndex = getSubprocessElementsRow(modelElements, multiSeqs);
					printArrayList(subprocessElements);
//					System.out.println();
					for(int k =j+1; k < multiSeqs[i].length; k++){
						if(multiSeqs[i][k] != null){
						postSubprocessElements.add(multiSeqs[i][k]);
					}		
				}
					multiSeqs = MultiProcessWithoutSubprocess(subprocessIndex,multiSeqs);
					printArrayList(postSubprocessElements);
					processSeq = ConcatinatingLists(processSeq, ConcatinatingLists(subprocessElements, postSubprocessElements)) ;
					multiSeqs = ArrayListConvereter(multiSeqs, processSeq, i);
//					System.out.println("modified board ");
//					printBoard(multiSeqs);
					j =0; //after adding the subprocess we reset the counter to check if there is other subprocesses in the bigger process
				}
				if(!ElementIsExists(multiSeqs[i][j],processSeq)){
				processSeq.add(multiSeqs[i][j]);		//adding the elements before the subprocess
			}
				}
		}	
		}
		System.out.println("aletered multiseqs");
		printBoard(multiSeqs);
		System.out.println();
//		printArrayList(processSeq);
		System.out.println();
		return null;
	}
	
	
	public static boolean ElementIsExists(String element, ArrayList<String> processSeq){
		for(int i=0; i< processSeq.size(); i++){
			if(element != null && processSeq.get(i).equals(element)){
				return true;
			}
		}
	...
	public static String[][] MultiProcessWithoutSubprocess(int subprocessIndex, String[][] multiseq){
		for(int i= 0; i< multiseq[subprocessIndex].length; i++){
			if(multiseq[subprocessIndex][i] != null){
				multiseq[subprocessIndex][i] = null;
			}
		}
		return multiseq;
	}
	
	public static ArrayList<String> ConcatinatingLists(ArrayList<String> firstList, ArrayList<String> toBeAdded){
		for(int i=0; i < toBeAdded.size(); i++){
			firstList.add(toBeAdded.get(i));
		}
		
		return firstList;
	}
	public static boolean isRuleIndex(String x,String[][]objInfo){
		for(int i = 0; i < objInfo.length; i++){
			if(objInfo[i][0] != null && objInfo[i][0].equals(x)){
				if(objInfo[i][1].equals("ST_BPMN_RULE_1")){
					return true;
				}
			}
		}
	...
	public static boolean isSubprocessIndex(String x,String[][]objInfo){
		for(int i = 0; i < objInfo.length; i++){
			if(objInfo[i][0] != null && objInfo[i][0].equals(x)){
				if(objInfo[i][1].equals("ST_BPMN_SUBPROCESS")){
					return true;
				}
			}
		}
	...
	public static String GetModelID(String x,String[][]objInfo){
		for(int i = 0; i < objInfo.length; i++){
			if(objInfo[i][0] != null && objInfo[i][0].equals(x)){
				if(objInfo[i][1].equals("ST_BPMN_SUBPROCESS")){
					return objInfo[i][5];
	...
	public static ArrayList<String> getModelElements(String modelID, String[][]ModelInfo){
		ArrayList<String> modelElements = new ArrayList<String>();
		for(int i = 0; i < ModelInfo.length; i++){
			if(ModelInfo[i][0] != null && ModelInfo[i][0].equals(modelID)){
				for(int j = 1; j < ModelInfo[i].length; j++){
					if(ModelInfo[i][j] != null){
					modelElements.add(ModelInfo[i][j]);
				}
				}		
			}
		}
	...
	public static ArrayList<String> getSubprocessElements(ArrayList<String> modelElements, String[][]multiSeqs){
		ArrayList<String> SubprocessElements = new ArrayList<String>();
		String singleElement = "";
		int subprocessIndex = 0;
		for(int k = 0; k < modelElements.size(); k++){
			singleElement = modelElements.get(k);
			for(int i = 0; i < multiSeqs.length; i++){
				for(int j = 0; j < multiSeqs[i].length; j++){
					if(singleElement!= null && multiSeqs[i][j] != null && singleElement.equals(multiSeqs[i][j])){
						subprocessIndex = i;
						break;
					}
				}
		...
		for(int l = 0; l < multiSeqs[subprocessIndex].length; l++){
			if(multiSeqs[subprocessIndex][l] != null){
				SubprocessElements.add(multiSeqs[subprocessIndex][l]);
			}
		}
	...
	public static int getSubprocessElementsRow(ArrayList<String> modelElements, String[][]multiSeqs){
...
//				  TokenData(objectsIDOfTheConnections,ObjectInfo, AddInfoArray,taskIndex, connectionsArray);
				  String[][] MultiSeq =  MultiSequance(ObjectInfo, connectionsArray);
				  ProcessSequance(MultiSeq, ModelElements, ObjectInfo, connectionsArray);
